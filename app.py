...
def post_alert():
    ...
    alert["created"] = datetime.now(TZ).isoformat()
...
def check_alerts():
    """Comprueba alertas contra precio actual y devuelve las disparadas."""
    alerts = load_alerts()
    if not alerts or not ESIOS_TOKEN:
        return jsonify({"triggered": []})
    today = date.today().isoformat()
    try:
        data = _fetch_esios(today)
        if not data:
            return jsonify({"triggered": []})
        hora_actual = datetime.now(TZ).hour
        precio_actual = data["hours"].get(hora_actual, 0)
        avg = data["avg"]
        triggered = []
        updated = False
        for a in alerts:
            tipo = a.get("tipo")
            umbral = float(a.get("umbral", 0))
            if tipo == "bajo" and precio_actual <= umbral and not a.get("triggered_today"):
                triggered.append({**a, "precio_actual": precio_actual, "avg": avg})
                a["triggered_today"] = today
                updated = True
            elif tipo == "alto" and precio_actual >= umbral and not a.get("triggered_today"):
                triggered.append({**a, "precio_actual": precio_actual, "avg": avg})
                a["triggered_today"] = today
                updated = True
            # Reset si es un día nuevo
            elif a.get("triggered_today") and a["triggered_today"] != today:
                a["triggered_today"] = None
                updated = True
        if updated:
            save_alerts(alerts)
        return jsonify({"triggered": triggered, "precio_actual": precio_actual, "avg": avg})
    except Exception as e:
        return jsonify({"error": str(e), "triggered": []}), 500

# ── Planificador semanal ──────────────────────────────────────────────────────
WEEKLY_PLAN_FILE = os.environ.get("WEEKLY_PLAN_FILE", "/config/weekly_plan.json")

def load_weekly_plan():
    try:
        with open(WEEKLY_PLAN_FILE) as f:
            return json.load(f)
    except Exception:
        return {}
...
def get_weekly_plan():
    return jsonify(load_weekly_plan())

@app.route("/api/weekly-plan", methods=["POST"])
def post_weekly_plan():
    plan = request.json or {}
    os.makedirs(os.path.dirname(WEEKLY_PLAN_FILE), exist_ok=True)
    with open(WEEKLY_PLAN_FILE, "w") as f:
        json.dump(plan, f, indent=2, ensure_ascii=False)
    return jsonify(plan)

# ── Widget resumen (página mínima para móvil) ─────────────────────────────────
@app.route("/resumen")
def resumen():
    settings = load_settings()
    now = datetime.now(TZ)
    ...
    min_p = None
    try:
        data = _fetch_esios(now.date().isoformat())
        if data:
            precio_actual = data["hours"].get(hora)
            avg = data["avg"]
            min_h = data["min_h"]
            min_p = data["min"]
    except Exception:
    ...
    diff_pct = round((precio_actual - avg) / avg * 100, 1) if precio_actual and avg else None
